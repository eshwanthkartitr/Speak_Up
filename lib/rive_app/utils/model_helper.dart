...
class ModelHelper {
  ...
  // since TFLite Flutter has compatibility issues with PyTorch models
  
  // Map to store label-to-index mapping
  ...
  static Future<String?> copyModelToDocuments(String assetPath, String filename) async {
    try {
  ...
  // Simulate model prediction for character recognition based on character set
  static String simulatePrediction(img.Image? image, {String characterSet = 'Tamil'}) {
    if (image == null || _cachedLabels == null || _cachedLabels!.isEmpty) {
      return '';
    }
    
    // Get range for the selected character set
    final range = _characterSetRanges[characterSet] ?? _characterSetRanges['Tamil']!;
    final startIndex = range['start']!;
    final endIndex = range['end']!;
    
    // Generate random index within the character set range
    final randomIndex = startIndex + math.Random().nextInt(endIndex - startIndex + 1);
    
    // Return a character based on the mapping
    return _indexToLabelMap[randomIndex] ?? '';
  }
  ...
  static List<String> generateWordSuggestions(String character, {String characterSet = 'Tamil'}) {
    if (character.isEmpty) return [];
    
    // Extract the base character without the transliteration
    final baseCharMatch = RegExp(r'(.*?)\(').firstMatch(character);
    final baseChar = baseCharMatch != null ? baseCharMatch.group(1)! : character;
  ...
  static Set<String> _generateTamilSuggestions(String baseChar, String transliteration) {
    // Dictionary of Tamil words based on characters
    final Map<String, List<String>> tamilDictionary = {
      // Vowels
      'அ': ['அம்மா (Mother)', 'அப்பா (Father)', 'அரசு (Government)', 'அன்பு (Love)', 'அழகு (Beauty)'],
      'ஆ': ['ஆடு (Goat)', 'ஆறு (River)', 'ஆசை (Desire)', 'ஆண்டு (Year)', 'ஆலயம் (Temple)'],
      'இ': ['இசை (Music)', 'இதயம் (Heart)', 'இரவு (Night)', 'இலை (Leaf)', 'இளம் (Young)'],
      'ஈ': ['ஈரம் (Wet)', 'ஈட்டி (Spear)', 'ஈசல் (Fly)', 'ஈர்ப்பு (Attraction)', 'ஈவு (Mercy)'],
      'உ': ['உணவு (Food)', 'உயிர் (Life)', 'உறவு (Relation)', 'உழைப்பு (Work)', 'உள்ளம் (Mind)'],
      'ஊ': ['ஊர் (Village)', 'ஊழல் (Corruption)', 'ஊக்கம் (Enthusiasm)', 'ஊமை (Mute)', 'ஊனம் (Disability)'],
      'எ': ['எழுத்து (Letter)', 'எறும்பு (Ant)', 'எண் (Number)', 'எரி (Burn)', 'எலி (Rat)'],
      'ஏ': ['ஏணி (Ladder)', 'ஏரி (Lake)', 'ஏழை (Poor)', 'ஏடு (Book)', 'ஏமாற்று (Deceive)'],
      'ஐ': ['ஐந்து (Five)', 'ஐயம் (Doubt)', 'ஐக்கியம் (Unity)', 'ஐவர் (Five People)', 'ஐப்பசி (Tamil Month)'],
      'ஒ': ['ஒளி (Light)', 'ஒற்றுமை (Unity)', 'ஒலி (Sound)', 'ஒப்பந்தம் (Agreement)', 'ஒதுக்கு (Exclude)'],
      'ஓ': ['ஓடு (Run)', 'ஓவியம் (Painting)', 'ஓசை (Noise)', 'ஓடை (Stream)', 'ஓய்வு (Rest)'],
      
      // Consonants
      'க': ['கல் (Stone)', 'கதவு (Door)', 'கடல் (Sea)', 'காடு (Forest)', 'கை (Hand)'],
      'ங': ['ஙனம் (This way)', 'ஙாயன் (The fat man)', 'ஙோடு (Bank)', 'ஙிலை (Place)', 'ஙாகம் (Dragon)'],
      'ச': ['சத்தம் (Sound)', 'சந்தை (Market)', 'சாவி (Key)', 'சிறகு (Wing)', 'சிரிப்பு (Laugh)'],
      'ஞ': ['ஞாயிறு (Sunday/Sun)', 'ஞானம் (Knowledge)', 'ஞாபகம் (Memory)', 'ஞாலம் (World)', 'ஞமலி (Wolf)'],
      'ட': ['டமாரம் (Drum)', 'டாக்டர் (Doctor)', 'டிக்கெட் (Ticket)', 'டீ (Tea)', 'டவுன் (Town)'],
      'ண': ['ணன் (A good person)', 'ணகை (Smile)', 'ணவு (Meal)', 'ணயம் (Good)', 'ணடை (Style)'],
      'த': ['தமிழ் (Tamil)', 'தண்ணீர் (Water)', 'தலை (Head)', 'தாய் (Mother)', 'தீ (Fire)'],
      'ந': ['நட்பு (Friendship)', 'நாடு (Country)', 'நீர் (Water)', 'நிலா (Moon)', 'நேரம் (Time)'],
      'ப': ['பல் (Tooth)', 'பறவை (Bird)', 'பாடம் (Lesson)', 'பூமி (Earth)', 'பேச்சு (Speech)'],
      'ம': ['மலர் (Flower)', 'மரம் (Tree)', 'மழை (Rain)', 'மனம் (Mind)', 'மாலை (Evening/Garland)'],
      'ய': ['யானை (Elephant)', 'யோகம் (Yoga)', 'யுத்தம் (War)', 'யாழ் (Harp)', 'யாத்திரை (Journey)'],
      'ர': ['ரசம் (Juice/Soup)', 'ரயில் (Train)', 'ரத்தம் (Blood)', 'ரவி (Sun)', 'ராஜா (King)'],
      'ல': ['லட்சியம் (Ambition)', 'லாபம் (Profit)', 'லீலை (Play)', 'லேசு (Easy)', 'லைட் (Light)'],
      'வ': ['வயல் (Field)', 'வாழை (Banana)', 'விண் (Sky)', 'வீடு (House)', 'வெற்றி (Victory)'],
      'ழ': ['ழகரம் (Letter ழ)', 'ழவி (Child)', 'ழரி (Ocean)', 'ழலை (Child Speech)', 'ழை (Cave/Entrance)'],
      'ள': ['ளம் (Youth)', 'ளன் (Young man)', 'ளி (Food)', 'ளை (Entertainment)', 'ளோ (Wonder)'],
      'ற': ['றவை (Assembly)', 'றணம் (Crowd)', 'றண்டு (Two)', 'றல் (House)', 'றை (Room)'],
      'ன': ['னம் (Self)', 'னி (Sweet)', 'னவி (Dream)', 'னைவு (Memory)', 'னிமை (Solitude)'],
    };
    
    // Add suggestions based on transliteration
    final translitBasedDict = {
      'a': ['அன்பு (Love)', 'அறிவு (Knowledge)', 'ஆடை (Dress)', 'அமைதி (Peace)'],
      'ā': ['ஆசை (Desire)', 'ஆனந்தம் (Joy)', 'ஆயுள் (Life)', 'ஆரோக்கியம் (Health)'],
      'i': ['இனிப்பு (Sweet)', 'இலக்கு (Goal)', 'இயற்கை (Nature)', 'இளமை (Youth)'],
      'ī': ['ஈகை (Charity)', 'ஈடுபாடு (Involvement)', 'ஈட்டம் (Wealth)', 'ஈசன் (God)'],
      'u': ['உறவு (Relationship)', 'உழைப்பு (Labor)', 'உணவு (Food)', 'உரிமை (Right)'],
      'e': ['எண்ணம் (Thought)', 'எதிர்காலம் (Future)', 'எளிமை (Simplicity)', 'எழுச்சி (Rise)'],
    };
  ...
  static Set<String> _generateArabicSuggestions(String baseChar, String transliteration) {
    ...
    final Map<String, List<String>> arabicDictionary = {
      ...
      'ت': ['تفاحة (Apple)', 'تمر (Dates)', 'تلميذ (Student)', 'تعليم (Education)'],
      'ث': ['ثلاثة (Three)', 'ثعلب (Fox)', 'ثقافة (Culture)', 'ثوب (Dress)'],
      'ج': ['جميل (Beautiful)', 'جبل (Mountain)', 'جامعة (University)', 'جديد (New)'],
      'ح': ['حب (Love)', 'حياة (Life)', 'حديقة (Garden)', 'حلو (Sweet)'],
      'خ': ['خبز (Bread)', 'خروف (Sheep)', 'خريف (Autumn)', 'خضار (Vegetables)'],
      ...
      'ص': ['صديق (Friend)', 'صباح (Morning)', 'صيف (Summer)', 'صورة (Picture)'],
      ...
      'ف': ['فم (Mouth)', 'فيل (Elephant)', 'فصل (Class/Chapter)', 'فرح (Happiness)'],
  ...
  static Set<String> _generateEnglishSuggestions(String character) {
    ...
    // Numbers
    for (int i = 0; i <= 9; i++) {
      englishDictionary['$i'] = ['Number $i', '${i}th place', '$i items', '$i o\'clock'];
    }
    ...
    if (englishDictionary.containsKey(character)) {
      ...
      suggestions.addAll(['Hello', 'World', 'Good', 'Day', 'Friend', 'Welcome']);
    }
